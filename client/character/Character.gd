...
func _physics_process(delta):
	...
	if !movement.hurt and gravity.not_rotating(delta):
	...
	if hit_something && lightbreak.direction.length() > 0:
...
func _process_item_forces(delta):
