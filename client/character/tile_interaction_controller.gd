...
func interact_with_solid_tiles(character: Character, lightning: LightbreakController) -> bool:
	...
	if abs(normal.x) > abs(normal.y):
		if normal.x > 0:
			game.tiles.on("left", tile_type, character, tilemap, coords)
			game.tiles.on("any_side", tile_type, character, tilemap, coords)
		else:
			game.tiles.on("right", tile_type, character, tilemap, coords)
			game.tiles.on("any_side", tile_type, character, tilemap, coords)
	else:
		if normal.y > 0:
			game.tiles.on("bottom", tile_type, character, tilemap, coords)
			game.tiles.on("any_side", tile_type, character, tilemap, coords)
			game.tiles.on("bump", tile_type, character, tilemap, coords)
		else:
			game.tiles.on("top", tile_type, character, tilemap, coords)
			game.tiles.on("any_side", tile_type, character, tilemap, coords)
