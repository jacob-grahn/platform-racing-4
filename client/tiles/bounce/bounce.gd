...
func bounce(player: Node2D, tilemap: TileMap, coords: Vector2i) -> void:
	...
	if is_moving_towards(player.position, player.previous_velocity, tile_position_global):
		# bounce, invert velocity
		player.velocity = player.previous_velocity.bounce(player.last_collision.get_normal())
		
		# add extra velocity
		player.velocity = player.velocity * (Vector2(1, 1) + (Vector2(BOUNCINESS, BOUNCINESS) * player.last_collision.get_normal().abs()))
