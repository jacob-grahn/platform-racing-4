...
func get_center_position(tile_map: TileMapLayer, coords: Vector2i) -> Vector2:
	return (Vector2(coords * Settings.tile_size) + Vector2(Settings.tile_size_half)).rotated(tile_map.rotation)


func deactivate(tile_map: TileMapLayer, coords: Vector2i) -> void:
	var atlas_coords: Vector2i = tile_map.get_cell_atlas_coords(coords)
	tile_map.set_cell(coords, 0, atlas_coords, Tile.DEACTIVATED_ALT_ID)


func set_visible(tile_map: TileMapLayer, coords: Vector2i, visible: bool) -> void:
	var atlas_coords: Vector2i = tile_map.get_cell_atlas_coords(coords)
	var alt_id: int
	if visible:
	...
	tile_map.set_cell(coords, 0, atlas_coords, alt_id)


func is_active(tile_map: TileMapLayer, coords: Vector2i) -> bool:
	var alt_id := tile_map.get_cell_alternative_tile(coords)
	return alt_id == Tile.VISIBLE_ALT_ID or alt_id == Tile.INVISIBLE_ALT_ID


func is_visible(tile_map: TileMapLayer, coords: Vector2i) -> bool:
	var alt_id := tile_map.get_cell_alternative_tile(coords)
	return alt_id == Tile.DEACTIVATED_ALT_ID or alt_id == Tile.VISIBLE_ALT_ID


func get_slug(tile_map: TileMapLayer, coords: Vector2i) -> String:
	return str(tile_map.get_path()) + "/" + str(coords)


func cooldown(tile_map: TileMapLayer, coords: Vector2i, seconds: float) -> bool:
