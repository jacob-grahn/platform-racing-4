...
const LOAD_ROW = preload("res://pages/editor/load_panel/load_row.tscn")
var selected_level = ""
...
func render() -> void:
	...
	var save_level_array = Helpers._list_saved_levels()
	for level_name in save_level_array:
		...
		if Helpers._get_current_level_name() == level_name:
...
func _delete_pressed():
	...
	Helpers._delete_level(selected_level)
	for child in row_holder.get_children():
