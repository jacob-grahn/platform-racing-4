...
func _check_session():
	nickname_label.text = Globals.Session.nickname


func _online_pressed():
	Main.set_scene(Main.LOBBY)
	#if Globals.Session.is_logged_in():
		#Main.set_scene(Main.LOBBY)
	#else:
		#if OS.has_feature('web'):
			#JavaScriptBridge.eval('window.location.replace("' + Globals.Helpers.get_base_url() + '/auth-ui/registration")')
