...
func init(p_question: String, answers_array: Array):
	question = p_question
	var error = $HTTPRequest.request(Globals.Helpers.get_base_url() + "/results?question=" + question.uri_encode())
	if error != OK:
...
func _on_voted(answer_str: String):
	...
	var error = $HTTPRequest.request(Globals.Helpers.get_base_url() + "/vote", [], HTTPClient.METHOD_POST, body)
	if error != OK:
