...
func set_tile(tile_map: TileMapLayer, coords: Vector2i, rotation_rad: float) -> void:
...
func finish():
	...
	var atlas_coords: Vector2i = tile_map.get_cell_atlas_coords(_coords)
	var alt_id: int = tile_map.get_cell_alternative_tile(_coords)
	await get_tree().physics_frame
	if _atlas_coords == atlas_coords:
		if alt_id == Tile.INVISIBLE_DEACTIVATED_ALT_ID:
			tile_map.set_cell(_coords, 0, _atlas_coords, Tile.DEACTIVATED_ALT_ID)
		else:
			tile_map.set_cell(_coords, 0, _atlas_coords, Tile.VISIBLE_ALT_ID)
	queue_free()
